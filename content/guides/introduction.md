...
So this can be large very quickly! But it is reasonable to expect that the number of solutions is relatively small in comparison.
...
ŝ = first(solutions(solve([f, l])))
# The normal space is now generated by
J_ŝ = [p([x,y]=>ŝ) for p in J]
# creat normal vector
λ̂ = randn(ComplexF64, 1)
# x̂ - û = J_ŝ' * λ̂ => u = x -  J_ŝ' * λ̂
û = ŝ - transpose(J_ŝ) * λ̂
```
...
```julia
result_û = monodromy_solve(C, [ŝ; λ̂], û; parameters=u)
```
...
```julia
result_û = monodromy_solve(C, [ŝ; λ̂], û; parameters=u, max_loops_no_progress=100, target_solutions_count=36)
```
...
```julia
solve(C, solutions(result_û); parameters=u, start_parameters=û, target_parameters=u₀)
```
