...

{
    ...
    "category": "section",
    "text": "The following polynomial system is what is called a binding polynomial in chemistry.using HomotopyContinuation\nimport DynamicPolynomials: @polyvar\n\n@polyvar w1 w2 w3 w4 w5 w6\n\nf = [11*(2*w1+3*w3+5*w5)+13*(2*w2+3*w4+5*w6),\n    11*(6*w1*w3+10*w1*w5+15*w3*w5)+13*(6*w2*w4+10*w2*w6+15*w4*w6),\n    330*w1*w3*w5+390*w2*w4*w6,\n    143*(2*w1*w2+3*w3*w4+5*w5*w6),\n    143*(6*w1*w2*w3*w4+10*w1*w2*w5*w6+15*w3*w4*w5*w6),\n    4290*w1*w2*w3*w4*w5*w6]Suppose we wanted to solve f(w)=a, wherea=[71, 73, 79, 101, 103, 107]To get an initial solution we compute a random forward solution with FixedPolynomials.jl.w_0 = vec(randn(6,1))\na_0 = evaluate(f, w_0)Now we set up the homotopy.H = StraightLineHomotopy(f-a_0, f-a)and compute a backward solution with starting value w_0 bysolve(H, w_0)By default the solve function uses SphericalPredictorCorrector as the pathtracking routing. To use the AffinePredictorCorrector instead we must writesolve(H, w_0, AffinePredictorCorrector())"
},
