...

{
    ...
    "category": "section",
    "text": "The following polynomial system is the example from Section 5.1 from[Decoupled molecules with binding polynomials of bidegree (n,2), Ren, Martini, Torres]It is called a binding polynomial.using HomotopyContinuation\nimport DynamicPolynomials: @polyvar\n\n@polyvar w[1:6]\n\nf = [\n    11*(2*w[1]+3*w[3]+5*w[5])+13*(2*w[2]+3*w[4]+5*w[6]),\n    11*(6*w[1]*w[3]+10*w[1]*w[5]+15*w[3]*w[5])+13*(6*w[2]*w[4]+10*w[2]*w[6]+15*w[4]*w[6]),\n    330*w[1]*w[3]*w[5]+390*w[2]*w[4]*w[6],\n    143*(2*w[1]*w[2]+3*w[3]*w[4]+5*w[5]*w[6]),\n    143*(6*w[1]*w[2]*w[3]*w[4]+10*w[1]*w[2]*w[5]*w[6]+15*w[3]*w[4]*w[5]*w[6]),\n    4290*w[1]*w[2]*w[3]*w[4]*w[5]*w[6]\n    ]Suppose we wanted to solve f(w)=a, wherea=[71, 73, 79, 101, 103, 107]To get an initial solution we compute a random forward solution with FixedPolynomials.jl.w_0 = randn(6)\na_0 = map(p -> p(w => w_0), f)Now we set up the homotopy.H = StraightLineHomotopy(f-a_0, f-a)and compute a backward solution with starting value w_0 bysolve(H, w_0)By default the solve function uses SphericalPredictorCorrector as the pathtracking routing. To use the AffinePredictorCorrector instead we must writesolve(H, w_0, AffinePredictorCorrector())"
},
